...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in vhs/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in vhs/kb\n"
...
UniValue makekeypair(const UniValue& params, bool fHelp)
{
    ...
    ret.push_back(Pair("PrivateKey", CBitcoinSecret(key).ToString()));
    return ret;
}
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"vhsaddress\"\n"
            "\nReturn information about the given vhs address.\n"
            "\nArguments:\n"
            "1. \"vhsaddress\"     (string, required) The vhs address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"vhsaddress\", (string) The vhs address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: VHS address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are vhs addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) vhs address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"vhsaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"vhsaddress\"  (string, required) The vhs address to use for the signature.\n"
