...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VHS)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_VHS4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
       ...
       // connect(ui->toggleObfuscation, SIGNAL(clicked()), this, SLOT(toggleObfuscation()));
        ...
        connect(ui->blabel_VHS, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("VHS")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
    ...
    int64_t BlockReward = GetBlockValue(CurrentBlock, tip_time);
    double BlockRewardVHS =  static_cast<double>(BlockReward)/static_cast<double>(COIN);
    //int64_t VHSSupply = chainActive.Tip()->nMoneySupply / COIN;
   ...
   /* for(const auto& it : Params().GetSubsidySwitchPoints(tip_time))
    ...
    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardVHS));
    //ui->label_VHSSupply_value->setText(QString::number(VHSSupply));
}
*/
