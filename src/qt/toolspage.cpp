 ...
 */
bool parseCommandLine_PageTools(std::vector<std::string>& args, const std::string& strCommand)
{
    enum CmdParseState {
        STATE_EATING_SPACES,
        STATE_ARGUMENT,
...
void RPCExecutor::request(const QString& command)
{
    ...
    if (!parseCommandLine_PageTools(args, command.toStdString())) {
        emit reply(ToolsPage::CMD_ERROR, QString("Parse error: unbalanced ' or \""));
        return;
    }
    if (args.empty())
    ...
    try {
        ...
        emit reply(ToolsPage::CMD_REPLY, QString::fromStdString(strPrint));
    } catch (UniValue& objError) {
        try // Nice formatting for standard-format error
        {
            int code = find_value(objError, "code").get_int();
            std::string message = find_value(objError, "message").get_str();
            emit reply(ToolsPage::CMD_ERROR, QString::fromStdString(message) + " (code " + QString::number(code) + ")");
        } catch (std::runtime_error&) // raised when converting to invalid type, i.e. missing code or message
        {                             // Show raw JSON object
            emit reply(ToolsPage::CMD_ERROR, QString::fromStdString(objError.write()));
        }
...
{
    switch (category) {
    case ToolsPage::CMD_REQUEST:
        return "cmd-request";
        break;
    case ToolsPage::CMD_REPLY:
        return "cmd-reply";
        break;
    case ToolsPage::CMD_ERROR:
        return "cmd-error";
        break;
    default:
        return "misc";
...
void ToolsPage::clear()
{
    ...
    message(CMD_REPLY, (tr("Welcome to the VHS RPC console.") + "<br>" +
...
void ToolsPage::setNumBlocks(int count)
{
...
void ToolsPage::setTrafficGraphRange(int mins)
{
...
void ToolsPage::showInfo()
{
    ui->tabWidget->setCurrentIndex(0);
    show();
}

void ToolsPage::showConsole()
{
    ui->tabWidget->setCurrentIndex(1);
    show();
}

void ToolsPage::showNetwork()
{
    ui->tabWidget->setCurrentIndex(2);
    show();
}

void ToolsPage::showPeers()
{
    ui->tabWidget->setCurrentIndex(3);
    show();
}

void ToolsPage::showRepair()
{
    ui->tabWidget->setCurrentIndex(4);
    show();
}

void ToolsPage::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void ToolsPage::showMNConfEditor()
{
