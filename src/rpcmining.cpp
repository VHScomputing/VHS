...
// Copyright (c) 2017-2018 The XDNA developers
...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or vhs.conf setting gen)\n"
    ...
    if (fGenerate && Params().MineBlocksOnDemand()) {
        ...
        while (nHeight < nHeightEnd) {
            ...
            while (!CheckProofOfWork(pblock->GetHash(), pblock->nBits)) {
                ...
                ++pblock->nNonce;
                LogPrintf("new nonce %d",pblock->nNonce);
            }
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "VHS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "VHS is downloading blocks...");
