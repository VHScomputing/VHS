...
#if defined(NDEBUG)
#error "VHS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvhs) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight, uint32_t nTime)
{
	...
	if(nHeight > 0 && nHeight <= 200) {
        return 1 * COIN;
	} else if (nHeight > 200 && nHeight <= 21600) {
		return 10 * COIN;
	} else if (nHeight > 21600 && nHeight <= 43200) {
		return 5 * COIN;
	} else if (nHeight > 43200 && nHeight <= 64800) {
		return 3 * COIN;
    ...
    if (block.IsProofOfStake()) {
        ...
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            ...
            if(nHeight > 1) { // exclude premine
                ...
                CScript scriptDevPubKeyIn  = CScript() << Params().xVHSDevKey() << OP_CHECKSIG;
                CScript scriptFundPubKeyIn = CScript() << Params().xVHSFundKey() << OP_CHECKSIG;
